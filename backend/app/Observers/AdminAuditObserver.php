...
class AdminAuditObserver
{
    private const SENSITIVE = [
        'google_id', 'mfa_secret', 'mfa_recovery_codes', 'mfa_last_step',
    ...
    private function record(Model $model, string $event, array $old, array $new): void
    {
        ...
        AuditLog::create([
            ...
            'subject_label' => $model->name ?? $model->question ?? $model->title ?? $model->email ?? $model->key ?? class_basename($model).' #'.$model->getKey(),
