...
class AuthController extends BaseController
{
    ...
    public function login(Request $request) {
        ...
        if ($user->mfa_secret) {
            $request->validate(['mfa_code'=>'nullable|string|max:64']);
            if(!$request->filled('mfa_code') || !app(\App\Services\AccountMfa::class)->consume($user,$request->mfa_code)) {
                return response()->json(['message'=>'Enter a fresh authenticator code or unused recovery code.', 'mfa_required'=>true],403);
            }
        }
    ...
    public function changePassword(Request $request) {
        ...
        if ($user->save()) {
            // Keep the current authenticated session; revoke all other API tokens.
