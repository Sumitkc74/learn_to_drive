...
class GovernmentNoticeImportController extends Controller
{
    ...
    {
        $notice = \Illuminate\Support\Facades\DB::transaction(function () use ($id) {
        $import = GovernmentNoticeImport::where('status', 'Pending')->lockForUpdate()->findOrFail($id);
        abort_if($import->assigned_to && $import->assigned_to !== auth()->id(),409,'This notice is assigned to another admin.');
        app(\App\Services\ContentAiScreening::class)->requireScreened($import);
    ...
    {
        $item=GovernmentNoticeImport::where('status', 'Pending')->findOrFail($id);
        abort_if($item->assigned_to && $item->assigned_to !== auth()->id(),409,'This notice is assigned to another admin.');
        $item->update(['status' => 'Rejected', 'reviewed_by' => auth()->id(), 'reviewed_at' => now()]);
