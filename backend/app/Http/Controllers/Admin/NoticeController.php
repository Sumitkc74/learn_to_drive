...
class NoticeController extends Controller
{
    ...
    public function allNotice(Request $request)
    {
        ...
        if (!$request->filled('status')) $query->where('status', '!=', 'Archived');
        $notices = AdminTable::paginate($query, $request,
    ...
    {
        Notice::findOrFail($id)->update(['status' => 'Archived']);
        return back()->with('success', 'Notice archived. It remains available in the archive.');
    }

    public function unarchiveNotice($id)
    {
        Notice::where('status', 'Archived')->findOrFail($id)->update(['status' => 'Draft', 'publish_at' => null, 'expires_at' => null]);
        return redirect()->route('editNotice', $id)->with('success', 'Restored as a draft. Review dates and content before publishing.');
    }
    ...
    private function rules(): array
    {
        return [
            ...
            'expires_at' => request('status') === 'Published' ? ['nullable', 'date', 'after:now'] : ['nullable', 'date'],
        ];
    }
}
