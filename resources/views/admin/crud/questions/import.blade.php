...
<div class="ltd-form-shell"><form class="ltd-form-card" action="{{ route('questionImport.store') }}" method="POST" enctype="multipart/form-data" data-add-form>@csrf
    <div class="ltd-form-card__intro"><i class="fas fa-file-excel"></i><div><h2>Import from CSV / Excel</h2><p>Import a prepared question set from a .csv or modern .xlsx workbook.</p></div></div>
    <div class="ltd-form-card__body">
        ...
        <section class="ltd-form-section"><h3 class="ltd-form-section__title">2. Prepare and validate</h3><ul class="text-muted mb-0"><li>Open the template in Excel, Google Sheets, or another spreadsheet editor.</li><li>Keep the first worksheet and header row unchanged.</li><li>Use A, B, C, or D for the correct option.</li><li>Save as UTF-8 CSV or Excel Workbook (.xlsx).</li><li>If any row is invalid, nothing will be imported.</li></ul></section>
        <section class="ltd-form-section"><h3 class="ltd-form-section__title">3. Upload CSV / Excel</h3><label class="ltd-field-label" for="question_file">Question file <span class="ltd-required">*</span></label><input id="question_file" name="question_file" type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" class="form-control-file @error('question_file') is-invalid @enderror" required><small class="form-text text-muted">Accepted formats: .csv and .xlsx. Maximum file size: 2 MB.</small>@error('question_file')<div class="alert alert-danger mt-3 mb-0">@foreach($errors->get('question_file') as $message)<div>{{ $message }}</div>@endforeach</div>@enderror</section>
