...
<div class="ltd-settings-grid">
    ...
    <form id="verification" class="ltd-settings-card" method="POST" action="{{ route('appSettings.update', 'verification') }}">@csrf @method('PATCH')
        ...
        <div class="ltd-settings-card__body"><div class="form-group"><label class="ltd-field-label" for="otp_resend_seconds">Minimum time between phone codes (seconds)</label><input id="otp_resend_seconds" type="number" name="otp_resend_seconds" value="{{ old('otp_resend_seconds', $settings['otp_resend_seconds']) }}" min="30" max="600" class="form-control" required>@error('otp_resend_seconds')<div class="text-danger small">{{ $message }}</div>@enderror</div></div><div class="ltd-settings-card__actions"><button class="btn btn-primary" type="submit"><i class="fas fa-save mr-1"></i>Save Verification</button></div>
    </form>
